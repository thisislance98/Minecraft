...
export class AssetManager {
    constructor(game) {
        ...
        this.blockProperties = {
            ...
            'dark_oak_wood': { hardness: 1.2 },
            'dark_oak_leaves': { hardness: 0.1 },
            'willow_wood': { hardness: 0.9 },
            'willow_leaves': { hardness: 0.1 },
            'acacia_wood': { hardness: 1.0 },
            'acacia_leaves': { hardness: 0.1 },
            ...
            'book_red': { hardness: 1.0 },
            'book_blue': { hardness: 1.0 },
            'book_green': { hardness: 1.0 },
            'book_brown': { hardness: 1.0 },
            'book_purple': { hardness: 1.0 },
            'book_mixed': { hardness: 1.0 },
    ...
    loadGeneratedTextures() {
        ...
        // Dark Oak
        const darkOakSide = this.getOrCreateMat('dark_oak_side');
        const darkOakTop = this.getOrCreateMat('dark_oak_top');
        this.registerBlockMaterials('dark_oak_wood', [darkOakSide, darkOakSide, darkOakTop, darkOakTop, darkOakSide, darkOakSide]);

        const darkOakLeaves = this.getOrCreateMat('dark_oak_leaves', true);
        this.registerBlockMaterials('dark_oak_leaves', darkOakLeaves);

        // Willow
        const willowSide = this.getOrCreateMat('willow_side');
        const willowTop = this.getOrCreateMat('willow_top');
        this.registerBlockMaterials('willow_wood', [willowSide, willowSide, willowTop, willowTop, willowSide, willowSide]);

        const willowLeaves = this.getOrCreateMat('willow_leaves', true);
        this.registerBlockMaterials('willow_leaves', willowLeaves);

        // Acacia
        const acaciaSide = this.getOrCreateMat('acacia_side');
        const acaciaTop = this.getOrCreateMat('acacia_top');
        this.registerBlockMaterials('acacia_wood', [acaciaSide, acaciaSide, acaciaTop, acaciaTop, acaciaSide, acaciaSide]);

        const acaciaLeaves = this.getOrCreateMat('acacia_leaves', true);
        this.registerBlockMaterials('acacia_leaves', acaciaLeaves);
        ...
        // Book Block Variants
        const bookRed = this.getOrCreateMat('book_red', false);
        this.registerBlockMaterials('book_red', [bookRed, bookRed, bookshelfTop, bookshelfTop, bookRed, bookRed]);

        const bookBlue = this.getOrCreateMat('book_blue', false);
        this.registerBlockMaterials('book_blue', [bookBlue, bookBlue, bookshelfTop, bookshelfTop, bookBlue, bookBlue]);

        const bookGreen = this.getOrCreateMat('book_green', false);
        this.registerBlockMaterials('book_green', [bookGreen, bookGreen, bookshelfTop, bookshelfTop, bookGreen, bookGreen]);

        const bookBrown = this.getOrCreateMat('book_brown', false);
        this.registerBlockMaterials('book_brown', [bookBrown, bookBrown, bookshelfTop, bookshelfTop, bookBrown, bookBrown]);

        const bookPurple = this.getOrCreateMat('book_purple', false);
        this.registerBlockMaterials('book_purple', [bookPurple, bookPurple, bookshelfTop, bookshelfTop, bookPurple, bookPurple]);

        const bookMixed = this.getOrCreateMat('book_mixed', false);
        this.registerBlockMaterials('book_mixed', [bookMixed, bookMixed, bookshelfTop, bookshelfTop, bookMixed, bookMixed]);
        ...
        // Ring Space Station Materials
        const stationHull = this.getOrCreateMat('station_hull');
        this.registerBlockMaterials('station_hull', stationHull);

        const stationRing = this.getOrCreateMat('station_ring');
        this.registerBlockMaterials('station_ring', stationRing);

        const stationWindow = this.getOrCreateMat('station_window', true);
        this.registerBlockMaterials('station_window', stationWindow);

        const stationCore = this.getOrCreateMat('station_core');
        this.registerBlockMaterials('station_core', stationCore);

        const stationLight = this.getOrCreateMat('station_light', true); // Use transparency for glow effect? Or just basic mat? Let's stick to Lambert for now but maybe emissive later.
        this.registerBlockMaterials('station_light', stationLight);

        const stationFloor = this.getOrCreateMat('station_floor');
        this.registerBlockMaterials('station_floor', stationFloor);

        const stationConsole = this.getOrCreateMat('station_console');
        this.registerBlockMaterials('station_console', stationConsole);

        // Glowstone (Bright yellow light source)
        const glowstone = this.getOrCreateMat('glowstone', false); // Opaque but bright
        // We can reuse 'station_light' texture or generate a new one. 
        // Let's expect 'glowstone' to be generated by TextureGenerator or fallback.
        this.registerBlockMaterials('glowstone', glowstone);

        // ===== ALIEN WORLD BLOCKS =====

        // Crystal World Blocks
        const crystalGround = this.getOrCreateMat('crystal_ground');
        this.registerBlockMaterials('crystal_ground', crystalGround);

        const crystalStone = this.getOrCreateMat('crystal_stone');
        this.registerBlockMaterials('crystal_stone', crystalStone);

        const crystalShard = this.getOrCreateMat('crystal_shard', true);
        this.registerBlockMaterials('crystal_shard', crystalShard);

        const crystalGlow = this.getOrCreateMat('crystal_glow', true);
        this.registerBlockMaterials('crystal_glow', crystalGlow);

        const crystalPlant = this.getOrCreateMat('crystal_plant', true);
        this.registerBlockMaterials('crystal_plant', crystalPlant);

        // Lava World Blocks
        const obsiditeGround = this.getOrCreateMat('obsidite_ground');
        this.registerBlockMaterials('obsidite_ground', obsiditeGround);

        const magmaStone = this.getOrCreateMat('magma_stone');
        this.registerBlockMaterials('magma_stone', magmaStone);

        const cooledLava = this.getOrCreateMat('cooled_lava');
        this.registerBlockMaterials('cooled_lava', cooledLava);

        const emberBlock = this.getOrCreateMat('ember_block', true);
        this.registerBlockMaterials('ember_block', emberBlock);
    ...
    getOrCreateMat(name, transparent = false) {
        ...
        // Gentle floating effect for Enterprise Blocks (no orbital movement - that's handled by SpaceShipManager)
        // NOTE: Removed shader-based orbital movement to fix frustum culling issues.
        // The shader was modifying vertices beyond the mesh's bounding box, causing parts to disappear.
        if (name.includes('enterprise')) {
            mat.onBeforeCompile = (shader) => {
                ...
                // Only gentle Y bobbing effect - orbital movement is handled by SpaceShipManager.update()
                shader.vertexShader = shader.vertexShader.replace(
                    '#include <begin_vertex>',
                    `
                    #include <begin_vertex>
                    
                    // Gentle Y bobbing only (no orbital movement to avoid frustum culling issues)
                    float floatY = sin(time * 0.5) * 0.2;
