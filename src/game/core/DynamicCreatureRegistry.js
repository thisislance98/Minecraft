...
export function registerDynamicCreature(definition) {
    ...
    try {
        ...
        // Also add to the game's allowedAnimalTypes so it's visible
        // (allowedAnimalTypes is initialized once from AnimalClasses at startup,
        // so dynamically registered creatures need to be added explicitly)
        if (window.__VOXEL_GAME__?.allowedAnimalTypes) {
            window.__VOXEL_GAME__.allowedAnimalTypes.add(name);
