...
function setupRoomEventHandlers(socket: any, roomId: string, worldId: string) {
    ...
    socket.on('soccer:request_host', () => {
        ...
        if (!room.soccerBallHost) {
            ...
            // Request ball state from host for this new player
            io.to(room.soccerBallHost).emit('soccer:request_ball_state', { requesterId: socket.id });
        }
    });

    // Handle ball state response from host (forwarded to requester)
    socket.on('soccer:ball_state_response', (data: { requesterId: string; pos: any; vel: any; scores: any }) => {
        // Send directly to the requester
        io.to(data.requesterId).emit('soccer:initial_state', {
            pos: data.pos,
            vel: data.vel,
            scores: data.scores
        });
    ...
    socket.on('world:reset', async () => {
        ...
        try {
            ...
            const isDev = !process.env.CLI_SECRET || process.env.CLI_SECRET === 'asdf123';
            const isOwner = userId && world.ownerId === userId;
            const isGlobalWorld = currentWorldId === 'global';

            // Allow reset if: dev mode, OR owner, OR (global world with no owner)
            const canReset = isDev || isOwner || (isGlobalWorld && !world.ownerId);
            ...
            console.log(`[Socket] World reset authorized: isDev=${isDev}, isOwner=${isOwner}, isGlobalWorld=${isGlobalWorld}`);
...
io.on('connection', (socket) => {
    ...
    // Note: Legacy join_game removed - all clients use join_world
