...
export class FallingTree {
    ...
    die() {
        ...
        // Spawn drops for each block at its current world position
        // This ensures drops appear along the fallen trunk
        this.mesh.updateMatrixWorld(true);

        // Iterate through original blocks and corresponding mesh children
        // Assuming mesh.children order matches constructMesh loop order
        for (let i = 0; i < this.blocks.length; i++) {
            const blockData = this.blocks[i];
            const meshChild = this.mesh.children[i];
            ...
            if (meshChild) {
                ...
                // Spawn drop
                this.game.spawnDrop(worldPos.x, worldPos.y + 0.2, worldPos.z, blockData.type);
            }
        }
